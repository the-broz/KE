...
class MainMenuSubState extends MusicBeatState
{
	...
	#if !switch
	var optionShit:Array<String> = ['story mode', 'freeplay', 'awards'];
	#else
	...
	override function create()
	{
		...
		black = new FlxSprite(-20,0).makeGraphic(Math.round(FlxG.width/4),Math.round(FlxG.height),FlxColor.BLACK);
		...
		for (i in 0...optionShit.length)
		{
			...
			if (firstStart)
				FlxTween.tween(menuItem,{y:  60 + (i * 160) + 50},1 + (i * 0.25) ,{ease: FlxEase.expoInOut, onComplete: function(flxTween:FlxTween) 
					{ 
						finishedFunnyMove = true; 
						changeItem();
					}});
			else
				menuItem.y =  60 + (i * 160) + 50;
		}
